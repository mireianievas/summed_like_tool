...
class FermiAnalysis(Events,EnergyMatrix):
    ...
    def plot_exposure_interpolate(self):
        self.exposure_interp = self.exposure.interp_to_geom(self.countsmap.geom.as_energy_true)
        f = plt.figure(figsize=(10, 3))
        ax1 = f.add_subplot(121)
        ax2 = f.add_subplot(122)
        self.exposure.slice_by_idx({"energy_true": 3}).plot(ax=ax1,add_cbar=True);
        self.exposure_interp.slice_by_idx({"energy_true": 0}).plot(ax=ax2, add_cbar=True);
    ...
    def set_edisp_interpolator(self):
        ...
        edisp_kernel = EDispKernel(axes=[drm_eaxis_true,drm_eaxis],
                                   data=drm_matrix)
        ...
        drm_interp = edisp_kernel.evaluate('linear',
                **{'energy':energy_reco,
                'energy_true':energy_true})
        
        self.edisp_interp_kernel = EDispKernel(axes=[axis_true,axis_reco],
