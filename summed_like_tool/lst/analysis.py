...
class Observations(Files):
    def create_datastore(self):
        self.log.info("Creating datastore from {}".format(self.lst_path))
        self.datastore = DataStore.from_dir(self.lst_path)
...
class EnergyAxes(Files):
     def set_energy_axes(self,eedges=None,emin=None,emax=None,nbins=None):
        ...
        if eedges == None:
            ...
            eedges = np.logspace(np.log10(float(emin/u.GeV)),
                                 np.log10(float(emax/u.GeV)),
                                 nbins)*u.GeV
        
        self.energy_axis      = MapAxis.from_energy_edges(eedges)
        self.energy_axis_true = self.energy_axis.copy(name="energy_true")
        
class LSTAnalysis(Observations,EnergyAxes):
    ...
    def create_safe_mask_min_aeff(self,aeff_percent=5):
        self.log.info("Create safe mask from minimum aeff max percentage")
        self.safe_mask_masker = SafeMaskMaker(methods=["aeff-max"], 
                                              aeff_percent=aeff_percent)
    ...
    def add_source_to_exclusion_region(self,src=None,radius=0.1*u.deg,reset=False):
        ...
        if len(self.exclusion_regions) == 0:
            ...
            exclusion_region = CircleSkyRegion(pos,0*u.deg)
    ...
    def plot_pointings(self):
        ...
        for k,obs in enumerate(self.observations_total):
            PointSkyRegion(SkyCoord(
                obs.obs_info['RA_PNT'],
                obs.obs_info['DEC_PNT'],
                frame="icrs", unit="deg")
            ).to_pixel(ax.wcs).plot(ax=ax, mfc='None',
